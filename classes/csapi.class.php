...
class csAPI{
	...
	public function updateServerFromPing($ping){
		...
		$this->log->timer('plugins');
		
		$this->log->timer('plugins1');
		
		// set server count -1 for all plugins that this server is using (we add them again later around line 152)
		$sp1 = $this->database->query("SELECT * FROM serverplugins WHERE serverID = '$server[ID]'");
		foreach($sp1 as $sp){
			$pcr .= $firstpc.' ID = \''.$sp['pluginID'].'\' ';
			$firstpc = 'OR';
		}
		if(count($sp1) > 0)$this->database->query("UPDATE plugins SET servers = servers - 1 WHERE $pcr");
		
		// delete server-plugin relationships for this server
		$this->database->query("DELETE FROM serverplugins WHERE serverID = '$server[ID]'");
		
		// getting info neede to rebuild plugins, players and corresponding relationships
		$dbp = $this->database->query("SELECT CONCAT_WS(',',defaultName,version) AS nm, ID FROM plugins");
		
		$dbpm = array();
		
		$this->log->timer('plugins1');
		
		$this->log->timer('plugins2');
		foreach($ping['info']['Plugins'] as $plugin){ // all plugins currently on the server
			
			$parts = explode(' ',$plugin);
			$vpnum = 1;
			$version = '';
			for($i=0;($i<count($parts) && $version == '');$i++){ // gets the first space-seperated part of plugin name that has a number in it which isn't the first one (to prevent plugins named like 'Ask8Ball' from breaking)
				if(strcspn($parts[$i], '0123456789') != strlen($parts[$i]) && $i != 0){
					$version = $parts[$i];
					$vpnum = $i;
				}	
			}
			...
			$pname = '';
			for($i=0;$i<$vpnum;$i++){ // builds the name (everything before the version)
				$pname.= $parts[$i].($i+1!=$vpnum ? ' ' : '');
			}
			
			array_push($dbpm,$pname.','.$version); // adds name and version for comparison with database
		}
		$this->log->timer('plugins2');
		
		$this->log->timer('plugins3');
		...
		$dpc1 = array_intersect($dbpm,$dbp);
		//DPC1: plugins that are on the current server and already in the database
		
		$dpc2 = array_diff($dbpm,$dbp);
		//DPC2: plugins that are on the current server and are NOT in the database
		...
		$this->log->timer('plugins4');
		
		foreach($dpc1 as $pl){ // formatting query string for +1 to server count for plugin
			if($firstfound == false){
				$toAdd .= ' OR ';
			}
			$pl = mysql_real_escape_string($pl);
			$parts  = explode(',',$pl);
			$toAdd .= '(defaultName = \''.$parts[0].'\' AND version = \''.$parts[1].'\')';
			$firstfound = false;
		...
		$this->log->timer('plugins5');
			
		foreach($dpc2 as $pl){ // inserting new plugins and formatting query for relationships to server
			
			$pl = mysql_real_escape_string($pl);
			$parts  = explode(',',$pl);
			$this->database->query("INSERT INTO plugins VALUES ('','$parts[0]','','$parts[1]','1',FALSE)");
			$iid = mysql_insert_id();
			if($iid != ''){
				if($first == false){
					$spvalues .= ',';
				}
				$spvalues .= '('.$server[ID].','.$iid.')';
				$first = false;
			}
		}
		...
		$this->log->timer('plugins6');
		
		if($firstfound == false){ // +1 to server count for plugins
			$this->database->query("UPDATE plugins SET servers = servers + 1 WHERE $toAdd");
		}
		if($first == false){ // adding relationships from plugins to server
			$this->database->query("INSERT INTO serverplugins VALUES $spvalues");
		}
